...
function StateMachine:addState(cfg)
    assert(type(cfg) == "table", "StateMachine:ctor() - invalid config")
    
    if cfg.callbacks~=nil and type(cfg.callbacks)=="table" then
        for name, callback in pairs(cfg.callbacks) do
            self.callbacks_[name] = callback;
        end
    end
    
    if cfg.events~=nil and type(cfg.events)=="table" then
