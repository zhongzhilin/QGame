...
local UIBottomItem  = class("UIBottomItem", function() return gdisplay.newWidget() end )

function UIBottomItem:ctor()
    
end

function UIBottomItem:CreateUI()
    local root = resMgr:createWidget("offical/offical_sec_role_node")
    self:initUI(root)
end

function UIBottomItem:initUI(root)
    self.root = root
    self:addChild(root)
    ccui.Helper:doLayout(self.root)
    
    uiMgr:configUITree(self.root)
    uiMgr:configUILanguage(self.root, "offical/offical_sec_role_node")
...
function UIBottomItem:setData(id,data)
...
function UIBottomItem:choose_call(sender, eventType)
