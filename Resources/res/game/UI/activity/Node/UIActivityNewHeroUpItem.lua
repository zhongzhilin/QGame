--region UIActivityNewHeroUpItem.lua
--Author : yyt
--Date   : 2018/02/06
...
local UIActivityNewHeroUpItem  = class("UIActivityNewHeroUpItem", function() return gdisplay.newWidget() end )
...
function UIActivityNewHeroUpItem:ctor()
    self:CreateUI()
end

function UIActivityNewHeroUpItem:CreateUI()
    local root = resMgr:createWidget("activity/upgradeHero_activity_node")
    self:initUI(root)
end

function UIActivityNewHeroUpItem:initUI(root)
    self.root = root
    self:addChild(root)
...
function UIActivityNewHeroUpItem:setData(data)

    if global.UIActivityNewHeroUpPanelCell then 
        global.UIActivityNewHeroUpPanelCell[data.id] = self
    end 
    
    self.data = data
    local isFinish = data.class == 5 and global.ActivityData:getNewActHeroUpgrade(data.id) == 1
...
function UIActivityNewHeroUpItem:initHeroItem(data)
...
function UIActivityNewHeroUpItem:setTBTouchEable(state)
...
function UIActivityNewHeroUpItem:onExit()

    if global.UIActivityNewHeroUpPanelCell then 
        global.UIActivityNewHeroUpPanelCell[self.data.id] = nil
    end 
end 


function UIActivityNewHeroUpItem:receiveHandler(sender, eventType)

    if not self.isCanGet then
        return global.tipsMgr:showWarning("cantGetReward")
    end

    local activityId = 31001
    global.ActivityAPI:getActivityReward(function (msg)
        -- body
        if not self.setData then return end
        global.ActivityData:updataNewActHeroById(self.data.id, 1)
        ...
        global.panelMgr:getPanel("UIActivityNewHeroUpPanel"):reFresh()
...
return UIActivityNewHeroUpItem
