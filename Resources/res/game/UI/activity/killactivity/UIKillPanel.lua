...
local UIKillPanel  = class("UIKillPanel", function() return gdisplay.newWidget() end )
...
function UIKillPanel:ctor()
    self:CreateUI()
end

function UIKillPanel:CreateUI()
    local root = resMgr:createWidget("activity/kill_activity/kill_panel")
    self:initUI(root)
end

function UIKillPanel:initUI(root)
    self.root = root
    self:addChild(root)
    ...
    uiMgr:configUILanguage(self.root, "activity/kill_activity/kill_panel")
...
--EXPORT_NODE_BEGIN
    ...
    self.rank = self.root.Node_export.bg.time_bg.rank_node_mlan_6.rank_mlan_3_export
    self.time_node = self.root.Node_export.bg.time_bg.time_node_mlan_4_export
    self.time = self.root.Node_export.bg.time_bg.time_node_mlan_4_export.time_export
    self.leagueReward = self.root.Node_export.bg.time_bg.leagueReward_mlan_56_export
...
function UIKillPanel:setData(activity)
...
function UIKillPanel:initBtn()
    ...
    for index, panel_index in pairs(self.data.btn) do
        ...
        uiMgr:addWidgetTouchHandler(btn, function(sender, eventType) 
            ...
            local call = global.ActivityData:getCallBack("UIKillPanel" , panel_index , self.data )
            if call then 
                call()
            end 
        end)
...
function UIKillPanel:updateOverTime()
...
function UIKillPanel:cleanTimer()
...
function UIKillPanel:close_panel(sender, eventType)

    global.panelMgr:closePanel("UIKillPanel")
end


function UIKillPanel:initTouch()
...
function UIKillPanel:onTouchMoved(touch, event)
...
function UIKillPanel:setCellTouch(state)
...
function UIKillPanel:onTouchBegan(touch, event)
...
function UIKillPanel:onTouchEnded(touch, event)
...
function UIKillPanel:onTouchCancel()
...
function UIKillPanel:infoCall(sender, eventType)
