...
local LordExpNode = class("LordExpNode", function() return gdisplay.newWidget() end )

function LordExpNode:ctor( callFunc )
    self.panel = global.speedPanel 
    self:initUI(callFunc)
end

function LordExpNode:onEnter() 
    
    self.lockRestTime = 0
end

function LordExpNode:initUI(callFunc)

    self.unLockQueue = callFunc
    self.panel.queueUnlock_node:setVisible(true)
    self.panel.queueUnlock_node.txt_leftTime_mlan_34:setString(luaCfg:get_local_string(10690))
    self.panel.restTime_node:setVisible(false)
    self.panel.timeline_node:setVisible(false)
    
end

function LordExpNode:setItemData(data)
    ...
    if data.itemId == 6 then
        ...
        if itemCount <= 0 then
            ...
            self.panel.speedUseText:setString(string.format(luaCfg:get_local_string(10691),luaCfg:get_item_by(data.itemId).typePara1))
...
function LordExpNode:sendGift()
    ...
    global.itemApi:itemUse(itemId,number,0,self.panel.buildingId,function(msg)
        ...
        global.tipsMgr:showWarningText(string.format(luaCfg:get_local_string(10691),luaCfg:get_item_by(itemId).typePara1 * number))
