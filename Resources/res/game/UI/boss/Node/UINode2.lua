...
local UINode2  = class("UINode2", function() return gdisplay.newWidget() end )

function UINode2:ctor()
    self:CreateUI()
end

function UINode2:CreateUI()
    local root = resMgr:createWidget("boss/boss_loop_monster2")
    self:initUI(root)
end

function UINode2:initUI(root)
    self.root = root
    self:addChild(root)
    ccui.Helper:doLayout(self.root)
    
    uiMgr:configUITree(self.root)
    uiMgr:configUILanguage(self.root, "boss/boss_loop_monster2")
...
--EXPORT_NODE_BEGIN
    ...
    self.road208 = self.root.run2_export.road208_export
    ...
    self.run5 = self.root.run5_export
    self.road501 = self.root.run5_export.road501_export
    self.road502 = self.root.run5_export.road502_export
    self.road503 = self.root.run5_export.road503_export
    self.road504 = self.root.run5_export.road504_export
    self.monster5 = self.root.monster5_export
    self.monster5 = UIBossItem.new()
    uiMgr:configNestClass(self.monster5, self.root.monster5_export)
...
function UINode2:checkRoad(index, data)
...
function UINode2:roadAction(isPlay, road, i, temp, data)
