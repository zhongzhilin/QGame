...
local UIISoldierTipsNode = require("game.UI.common.UIISoldierTipsNode")


local UIISoldierTipsControl  = class("UIISoldierTipsControl")
local resMgr = global.resMgr
local uiMgr = global.uiMgr
local luaCfg = global.luaCfg
function UIISoldierTipsControl:ctor()

end

function UIISoldierTipsControl:setdata(original,data,target)
...
function UIISoldierTipsControl:setTips() 

	if self.target and self.target.child_tips then  -- 所有items  都用一个tips

		self.tips = self.target.child_tips

	elseif not self.tips then

		local desc_node  =UIISoldierTipsNode.new()

		self.target:addChild(desc_node)

		self.target.child_tips = desc_node
...
function UIISoldierTipsControl:bindTips(pos, isinit)
	...
	local action = cc.Sequence:create(
		...
		, cc.CallFunc:create( function ()
			if self.isTouch == 1 then 
				...
				self.tips:setData(self.data, true)
			end 
		end 	
		)
	)
...
function UIISoldierTipsControl:cleantimer()
	if self.tips then
		self.tips:stopAllActions()
	end
end

function UIISoldierTipsControl:setIsDelay(isdelay)
	self.isdelay= isdelay
end 

local  part_height = 30 
-- local  gdisplay = {width =720 ,height =1280}
function UIISoldierTipsControl:adjustPosition(touchpoint)
...
-- tips
function UIISoldierTipsControl:setTipsContontSize(size)
	self.tipsContentSize = clone(size)
	-- dump(self.tipsContentSize,"默认向上显示")
end 

function UIISoldierTipsControl:adjustPositionY(touchpoint)
...
function UIISoldierTipsControl:initTouch()
...
function UIISoldierTipsControl:onTouchCancel(touch , event)
	self:cleantimer()
	if self.tips   then 
		self.tips:stopAllActions()
		self.tips:setVisible(false)
...
function UIISoldierTipsControl:onTouchMoved(touch, event)
...
function UIISoldierTipsControl:onTouchBegan(touch, event)
...
function UIISoldierTipsControl:setCheckMode(checkmode)
	self.checkmode =  checkmode 
end

function UIISoldierTipsControl:CheckContrains(rect , point)
	  if cc.rectContainsPoint(rect,point) then 
	    	self:bindTips(self.beginPoint)
	  end  
end

function UIISoldierTipsControl:onTouchEnded(touch, event)
	if self.tips   then 
		self.tips:stopAllActions()
		self.tips:setVisible(false)
	end 
...
function UIISoldierTipsControl:isExitNoHideTips(state)
...
function UIISoldierTipsControl:ClearEventListener()
	self.isdelay= nil 
