...
local FORMAT_HEADER_LINE       = "| %-35s: %-30s: %-8s: %-12s: %-12s: %-12s: %-12s|\n"
local FORMAT_OUTPUT_LINE       = "| %s: %-12s: %-12s: %-12s: %12.7f|\n"
...
local FORMAT_TITLE             = "%-35.35s: %-30.30s: %-8s"
...
function ProFi:getTitleFromFuncInfo( funcInfo )
	...
	if #source > 30 then  source = "..." .. string.sub(source, #source-30) end
...
function ProFi:writeProfilingReportsToFile( reports, file )
	...
	local header = string.format( FORMAT_HEADER_LINE, "FILE", "FUNCTION", "LINE", "TIME", "RELATIVE", "CALLED", "ONETIME" )
	file:write( header )
 	for i, funcReport in ipairs( reports ) do
		...
		local oneTime  		= 0
		if funcReport.count > 0 then 
			oneTime = funcReport.timer/funcReport.count * 1000
		end
		local outputLine    = string.format(FORMAT_OUTPUT_LINE, funcReport.title, timer, relTime, count, oneTime )
