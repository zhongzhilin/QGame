...
function HeroExpNode:initUI(callFunc)
    ...
    self.panel.queueUnlock_node.txt_leftTime_mlan_34:setString(luaCfg:get_local_string(10467))
...
function HeroExpNode:setItemData(data)
    ...
    if data.itemId == 6 then
        ...
        if itemCount <= 0 then
            ...
            self.panel.speedUseText:setString(string.format(luaCfg:get_local_string(10466).."%s",luaCfg:get_item_by(data.itemId).typePara1))
            self.panel.speedTime:setString("")
        else
                
            local para = luaCfg:get_item_by(data.itemId).typePara1
            local heroData = global.heroData:getHeroDataById(self.panel.buildingId)
            local cutAge = heroData.hp - heroData.serverData.lCurHP
            local maxCount = math.min(itemCount,math.ceil(cutAge / para))
            ...
            self.panel.speedUseText:setString(string.format(luaCfg:get_local_string(10466).."%s",para))
...
function HeroExpNode:sendGift()
    ...
    global.itemApi:itemUse(itemId,number,0,self.panel.buildingId,function(msg)
        ...
        global.tipsMgr:showWarningText(string.format(luaCfg:get_local_string(10466).."%s",luaCfg:get_item_by(itemId).typePara1 * number))
