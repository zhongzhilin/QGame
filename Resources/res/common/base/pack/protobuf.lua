...
function _decode_type_meta:__index(key)
	--解决另外发现decode后的lua table在访问不存在的字段会报assert,
...
local function expand_table(tbl, ret)   
    for k,v in pairs(tbl) do
        if type(v) == "table" then
            ret[k] = {}
            if v._CObj ~= nil and v._CType ~= nil then
                --expand(v) 调用 v._CObj 时已经执行 expand 无需再执行
                expand_table(v, ret[k])
            else
                expand_table(v, ret[k])
            end
        else
...
function decodeAll(typename, buffer, length)
	local ret = {}
	local ok = c._decode(P, decode_message_cb , ret , typename, buffer, length)
	if ok then
		local tbl = setmetatable(ret , default_table(typename))
                local result = {}
		expand_table(tbl, result)
		return result
	else
		return false , c._last_error(P)
	end
