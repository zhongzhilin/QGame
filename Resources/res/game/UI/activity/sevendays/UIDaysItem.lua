...
local UIDaysItem  = class("UIDaysItem", function() return gdisplay.newWidget() end )

function UIDaysItem:ctor()
    
end

function UIDaysItem:CreateUI()
    local root = resMgr:createWidget("activity/sevendays/sevenday_btn")
    self:initUI(root)
end

function UIDaysItem:initUI(root)
    self.root = root
    self:addChild(root)
    ccui.Helper:doLayout(self.root)
    
    uiMgr:configUITree(self.root)
    uiMgr:configUILanguage(self.root, "activity/sevendays/sevenday_btn")
...
function UIDaysItem:setData(data , call)
    ...
    self.call  = call 
     ...
     uiMgr:setRichText(self, "day", 50140 ,{num = self.data.index})
...
function UIDaysItem:updatePointState()
...
function UIDaysItem:updateSelect(index)
...
function UIDaysItem:onClick(sender, eventType)
...
function UIDaysItem:onClick2(sender, eventType)
