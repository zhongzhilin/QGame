...
local InfoNode5  = class("InfoNode5", function() return gdisplay.newWidget() end )

function InfoNode5:ctor()
    self:CreateUI()
end

function InfoNode5:CreateUI()
    local root = resMgr:createWidget("city/building_5info")
    self:initUI(root)
end

function InfoNode5:initUI(root)
    self.root = root
    self:addChild(root)
    ccui.Helper:doLayout(self.root)
    
    uiMgr:configUITree(self.root)
    uiMgr:configUILanguage(self.root, "city/building_5info")
...
--EXPORT_NODE_END
    uiMgr:initScrollText(self.num3) -- 士兵技能解锁
...
function InfoNode5:setData( data )
