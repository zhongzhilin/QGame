...
local UIWildTipsNode2  = class("UIWildTipsNode2", function() return gdisplay.newWidget() end )

function UIWildTipsNode2:ctor()
    self:CreateUI()
end

function UIWildTipsNode2:CreateUI()
    local root = resMgr:createWidget("common/wild_pro_tips_node")
    self:initUI(root)
end

function UIWildTipsNode2:initUI(root)
    self.root = root
    self:addChild(root)
    ccui.Helper:doLayout(self.root)
    
    uiMgr:configUITree(self.root)
    uiMgr:configUILanguage(self.root, "common/wild_pro_tips_node")
...
--EXPORT_NODE_BEGIN
    ...
    self.pic = self.root.portrait_node_export.pic_export

--EXPORT_NODE_END
end

function UIWildTipsNode2:setData(data)
    
    local wildMonsterData = luaCfg:get_wild_monster_by(data.kindType)
    local id = wildMonsterData.defensesoldier[1] or wildMonsterData.attacksoldier[1]
    local proData = luaCfg:get_wild_property_by(id)
    local world_surface = luaCfg:get_world_surface_by(wildMonsterData.file)
    ...
    global.panelMgr:setTextureFor(self.pic, world_surface.uimap)
    self.pic:setScale(world_surface.UISize)
...
function UIWildTipsNode2:getContentSize() --qaq
